...
class ShoppingCartController extends Controller
{
    public function showCart() {
        ...
        if ($cart->isEmpty()) {
            error_log('niiiiiiiiiiiiiiiiiiiiiiiiiiiiggers');
            return view('user.shopping_cart')->with('info', 'Brak produktów w koszyku.');
        }
    ...
    public function postOrder(Request $request) {
        ...
        $date = new DateTime;
        $invoice = true;
        if($request->input('invoice') == null) $invoice = false;
        $order = new Order([
            'user_id' => $user->id,
